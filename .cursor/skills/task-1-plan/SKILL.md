...
8. Do not plan writes into the developer's real `~/.claude/skills` or
   `~/.cursor/skills` unless the task is an explicit live-install test the
   human asked for.
9. If the task is ambiguous, stop and ask. If it is *exceptionally* hard even
   with a detailed plan, add **`Execute model recommendation: large`** with a
   one-line why; otherwise omit.  
...
### Tests to add
- Shell cases against a temp HOME / pack-check cases …
