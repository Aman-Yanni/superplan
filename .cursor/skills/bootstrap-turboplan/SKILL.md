...
6. Do not implement product features. Specifically:
   - NEVER create the published pack (`skills/<name>/SKILL.md` bodies),
     installer logic in `install.sh`, or hub templates under `templates/hub/`.
     Those belong to phase tasks via `/task-2-execute`.
   - NEVER run `go mod init` or scaffold a Node/Python app. Superplan is a
     Markdown + POSIX shell pack.
   - In-scope (NOT product code): root Makefile, .shellcheckrc, lefthook.yml,
...
7. Bootstrap AC requires **git repo + verify gate files present + lefthook installed**:
   root `Makefile` with `verify` (= lint + test + pack-check/build), `.shellcheckrc`,
   and `lefthook install` succeeded.
   **Do not require `make verify` to pass yet** — T01 creates the skeleton
   (`install.sh --help`, `tests/run.sh`); verify first passes in T01.  
...
2. **Ship the verify gate at project root** (adapt seeds; do not leave them only
   under `planning/`):
   - Root **`Makefile`** must expose **`verify`** that runs **lint + test + build**.
     Document `make verify` and `make install-hooks` in hub Build & Run **and** README.
   - Root **`lefthook.yml`**: pre-commit → `make verify` (not ad-hoc partial checks).
   - Stack lint config for Superplan: **`.shellcheckrc`**. Do not ship `.golangci.yml`.
3. **Superplan Makefile shape:**
   - `lint` — shellcheck on `install.sh` and `tests/*.sh`
   - `test` — `tests/run.sh` (fake HOME; never the real skill dirs)
   - `build` — pack-check of `skills/*/SKILL.md`
   - `verify` — `lint` + `test` + `build`
   - `fmt`, `clean`, `install-hooks`, `help`
   No `build-all` / Go cross-compile — this is not a Go app.
...
5. Bootstrap AC **fails** if any of these files are missing, if `verify` target is not
   ...
   **Do not require `make verify` to pass** — T01 creates the skeleton.
...
1. Install lint/format tools at current stable (shellcheck, lefthook) via the
   host package manager. Record versions in hub Build & Run + README Status.
   Node is only needed later for `npx skills`; do not make Superplan a Node app.
2. If policy or the machine blocks upgrades, list under **Bootstrap concerns**
...
It creates the minimal runnable tree so that `make verify` passes:
- For Superplan: `install.sh` that prints `--help`, `tests/run.sh`, pack-check,
  and `skills/` directory (empty pack is OK).
- No published skill bodies, no real global install — just the skeleton.
- After T01, `make verify` must pass (lint + test + pack-check).  
...
- [ ] T01 is the skeleton bootstrap task (`install.sh --help`, tests, pack-check, `make verify` passes)  
