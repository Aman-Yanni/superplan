...
description: >
  ...
  commit, push if origin exists (opt out with --no-push; never invent a remote),
  give manual test commands, checkout next stub-stem branch. Manual only —
...
4a. **Verify tooling presence (hard abort):** Same as `/task-2-execute` — require
    root `Makefile` with `verify` (lint+test+build), `.shellcheckrc`,
    ...
    Never treat a single script run as the complete gate.
...
6. **Push if `origin` exists** after a successful commit (`git push -u origin HEAD`
   when upstream is unset). Skip push if **`--no-push`** was provided **or**
   there is no remote — do **not** create a GitHub remote. Never merge PRs
   unless the human opts in.
...
Be concrete for this stack (`./install.sh --help`, a dry-run into a temp HOME,
`ls` of skill dests, `/grill-me` visible in Claude and Cursor after live install).
